...
import { getSnapIndex, getSnapScrollTop } from '@/lib/viewport'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    function goTo(idx) {
      ...
      if (idxRef.current === 0 && idx === TOTAL - 1) {
        fadeLoop(getSnapScrollTop(TOTAL - 1), TOTAL - 1)
        return
      }
      ...
      tweenRef.current = gsap.to(el, {
        scrollTop: getSnapScrollTop(idx),
        duration: 1.0,
        ease: 'power3.inOut',
    ...
    function canScrollInDirection(container, direction) {
      if (direction > 0) {
    ...
    function onTouchStart(e) {
      ...
      touchScrollable = getScrollableAncestor(e.target)
    ...
    function onTouchEnd(e) {
      ...
      const scrollable = touchScrollable ?? getScrollableAncestor(e.target)
      if (scrollable) {
        if (Math.abs(scrollable.scrollTop - touchStartScrollTop) > 8) return
        const direction = dy > 0 ? 1 : -1
        if (canScrollInDirection(scrollable, direction)) return
      }
    ...
    function onScroll() {
      idxRef.current = Math.max(0, Math.min(TOTAL - 1, getSnapIndex(el.scrollTop)))
    }

    function canScrollInternally(target) {
      return getScrollableAncestor(target) !== null
    }

    function onTouchMove(e) {
      if (busyRef.current || canScrollInternally(e.target)) return
      e.preventDefault()
    }
  ...
  return (
    <>
      ...
      <Navbar />
      <main ref={mainRef} className="scrollRoot">
        <div>
          <VideoIntro />
          <HeroSection />
          <AboutSection />
