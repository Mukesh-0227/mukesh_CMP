...
import { getSlideHeight, getSnapScrollTop } from '@/lib/viewport'
import {
...
function handleViewProjects() {
  ...
  if (scroller) gsap.to(scroller, { scrollTop: getSnapScrollTop(3), duration: 1.0, ease: 'power3.inOut' })
}
...
export default function PublicationsFooterSection() {
  ...
  useEffect(() => {
    ...
    if (!isMobile && canvas && videoEl) {
      ...
      videoEl.src       = profile.assets.footerVideo
      videoEl.muted     = true
    ...
    <div ref={wrapperRef} className={styles.wrapper}>
      <div ref={stickyRef} className={styles.sticky}>
        ...
        <div className={styles.mobileFooterBg}>
          <Image
            src={profile.assets.mobileFooter}
            alt=""
            fill
            quality={100}
        ...
        <div ref={pubContentRef} className={styles.pubContent}>
          <span className={styles.watermark} aria-hidden>LEARN</span>

          <div className={styles.pubHero}>
            <p  ref={labelRef}   className={styles.label}>{content.publications.label}</p>
            <h2 ref={headingRef} className={styles.heading}>{content.publications.heading}</h2>
          </div>
