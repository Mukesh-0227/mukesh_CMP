...
import { getSlideHeight } from '@/lib/viewport'
...
function scrollNext() {
  ...
  if (main) main.scrollTo({ top: getSlideHeight(), behavior: 'smooth' })
}
...
export default function VideoIntro() {
  ...
  // Pause intro videos when scrolled away; resume when back on slide 0
  useEffect(() => {
    const scroller = document.querySelector('main')
    if (!scroller) return

    function setVideosPlaying(play) {
    ...
    function syncWithScroll() {
      const onIntro = scroller.scrollTop < getSlideHeight() * 0.5
      ...
      const mainV = videoRef.current
      if (mainV && !mainV.paused) {
        shouldResumeRef.current = true
      }
      setVideosPlaying(false)
    }

    scroller.addEventListener('scroll', syncWithScroll, { passive: true })
    syncWithScroll()

    return () => scroller.removeEventListener('scroll', syncWithScroll)
  }, [])
  ...
  function handleEnded() {
    const main = document.querySelector('main')
    if (main && main.scrollTop < getSlideHeight() * 0.4) scrollNext()
  }
