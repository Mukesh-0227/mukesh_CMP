...
export default function WorkExperienceSection() {
  ...
  // Capture collapsed bullet heights for desktop hover expand
  useEffect(() => {
  ...
  function showMobileContent() {
    ...
    bulletListRefs.current.forEach(ul => {
      if (ul) gsap.set(ul, { maxHeight: 'none', clearProps: 'maxHeight' })
    })
  }
  ...
  useEffect(() => {
    ...
    if (isMobileViewport()) {
      const id = requestAnimationFrame(showMobileContent)
      return () => cancelAnimationFrame(id)
    }
    ...
    function onScroll() {
      const inRange = Math.abs(scroller.scrollTop - section.offsetTop) < getSlideHeight() * 0.5
  ...
  useEffect(() => {
    function onResize() {
      if (isMobileViewport()) showMobileContent()
    }
  ...
  return (
    <section ref={sectionRef} className={styles.section}>
      ...
      <div className={styles.bgImg} aria-hidden>
        <Image
          src={profile.assets.workExperience}
          alt=""
          fill
          quality={100}
          sizes="100vw"
      ...
      <div className={styles.timeline}>
        <p className={styles.scrollHint} aria-hidden>Scroll for all experience</p>
            ...
            {EXPS.map((exp, i) => (
              ...
              >
                ...
                >
                  ...
                  <h2 className={styles.company} title={exp.company}>
                    <span className={styles.companyFull}>{exp.company}</span>
                    {exp.companyShort && (
