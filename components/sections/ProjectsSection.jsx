...
import { getSlideHeight, isMobileViewport } from '@/lib/viewport'
...
export default function ProjectsSection() {
  ...
  useEffect(() => {
    ...
    // ── Mobile: sync horizontal track to main snap scroll (slides 3 → 4) ──
    if (isMobileViewport()) {
      contentRefs.current.forEach((el, i) => {
        if (el && i > 0) gsap.set(el, { opacity: 0, y: 20 })
      })

      function syncMobileProjects() {
        const projectsWrap = section.parentElement
        const vh = getSlideHeight()
        const rel = (scroller.scrollTop - projectsWrap.offsetTop) / vh
        const clampedRel = Math.max(0, Math.min(n - 1, rel))
        const progress = n > 1 ? clampedRel / (n - 1) : 0
        const activeIdx = Math.min(n - 1, Math.round(clampedRel))

        gsap.set(track, {
          xPercent: -progress * ((n - 1) / n) * 100,
          x: 0,
        })
      ...
      scroller.addEventListener('scroll', syncMobileProjects, { passive: true })
      window.addEventListener('resize', syncMobileProjects)
      syncMobileProjects()
      return () => {
        scroller.removeEventListener('scroll', syncMobileProjects)
        window.removeEventListener('resize', syncMobileProjects)
      }
    }

    // ── Desktop: horizontal ScrollTrigger scrub ──
    contentRefs.current.forEach((el, i) => {
  ...
  return (
    <div
      ...
      style={{ '--project-slides': PROJECTS.length }}
    >
      <section ref={sectionRef} className={styles.section}>
        ...
        {/* Progress bar + mobile hint */}
        <div className={styles.bottomUI}>
          <p className={styles.swipeHint} aria-hidden>
            {slideIdx < PROJECTS.length - 1 ? 'Swipe up for next project' : 'Swipe up for experience'}
          </p>
